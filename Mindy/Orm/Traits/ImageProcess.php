...
trait ImageProcess
{
    ...
    public function resize($img, $width, $height, $method)
    {
        ...
        if ($method == 'resize') {
        ...
        } elseif ($method == 'adaptiveResizeFromLeft') {
            ...
            $thumbnail->usePalette($img->palette());
            $thumbnail->strip();

            $ratio = max($ratios);
            if (!$imageSize->contains($box)) {
                $box = new Box(
                    min($imageSize->getWidth(), $box->getWidth()),
                    min($imageSize->getHeight(), $box->getHeight())
                );
            } else {
                $imageSize = $thumbnail->getSize()->scale($ratio);
                $thumbnail->resize($imageSize, ImageInterface::FILTER_UNDEFINED);
            }
            $img = $thumbnail->crop(new Point(0, 0), $box);
        }
